...
# since f returns N4, L4 and Q3, create a derivative lambda f_N4 that returns only N4
...
print "Wallenius fit yields H_Si={} and H_Al={} with ssq={}".format(fit[0], fit[1], chi_sq(N4, fitN4))
...
ax.plot(X, np.minimum(1, Na2O/Al2O3), label="$\mathrm{L}_4$ expected", marker='o', color='black', lw=1)
ax.plot(X, fitL4, label="$\mathrm{L}_4$ predicted", marker='o', color='purple', lw=1)
ax.plot(X, fitQ3, label="$\mathrm{Q}_3$ predicted", marker='o', color='green', lw=1)
textstr = '$\Delta$H$_{{^{{[3]}}\mathrm{{Si}}}}$ = ${}$\n$\Delta$H$_{{^{{[4]}}\mathrm{{Al}}}}$ = ${}$'.format(str(round(fit[0]*10000)/10000)[:6], str(round(fit[1]*10000)/10000)[:7])
...
ax.set_title("Wallenius 3-state speciation ( $^{[4]}\mathrm{B}$, $^{[3]}\mathrm{Si}$, and ${}^{[4]}\mathrm{Al}$ )", y=1.03)
